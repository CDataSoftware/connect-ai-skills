...
- **Prefer literal `true`/`false` on boolean columns.** Some boolean columns reject numeric `1`/`0` (a few such as `IsDeleted` do accept them, but not all) — e.g. `WHERE [Open/Closed] = 1` fails with `"Cannot decipher where clause as Boolean."` Use `WHERE [IsDeleted] = false`, `WHERE [Open/Closed] = true`. This applies to both SELECT filters and UPDATE SET clauses.
