...
- **Information schema User/Group views may return 403 depending on PAT scopes.** `[Information].[Users]`, `[Information].[Groups]`, `[Information].[GroupCollaborators]`, and `[Information].[GroupMembers]` require enterprise-level API scopes on the Personal Access Token used for authentication. If you get an HTTP 403 Forbidden on these views, check the selected Scopes on the PAT in the Airtable developer hub — the token may need additional permissions (e.g. `user.email:read`, enterprise admin scopes). Base-level collaborator data is still accessible via the `[T_Collaborators]` view on each table, which does not require these scopes.
