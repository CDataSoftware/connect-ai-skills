...
- **Reference fields are flattened quads**: `<entity>_Id`, `<entity>_Key`, `<entity>_Name`, `<entity>_Href`. For example `customer_Id` / `customer_Key` / `customer_Name`, `glAccount_Id` / `glAccount_Key` / `glAccount_Name`, `vendor_Id` / `vendor_Key`. Filter or join on `_Key` (the system key) or `_Id` (the business ID); `_Key` is the most universally reliable. `_Name` is the display value; `_Href` is the REST API link.
...
- **Reference fields are `_Id` / `_Key` / `_Name` / `_Href` quads.** Filter or join on `_Key` (system key) or `_Id` (business ID); prefer `_Key` as the most reliable. `_Name` is display-only.
