...
> **Where `$jwt` comes from:** the skill's normal Auth0 sign-in — `$jwt = node scripts/connect-cli.mjs token` (PKCE sign-in, no secret). Admin is CLI-only (see [authentication.md](authentication.md)). The CLI's `oauth-start` / `oauth-finish` commands wrap this handshake — see [cli.md](cli.md).
