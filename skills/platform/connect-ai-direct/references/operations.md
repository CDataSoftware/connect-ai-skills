...
This skill does NL→SQL **client-side**: discover the schema first, then write the SQL yourself and run it via `/api/query`. The portal's `/api/ui/openai/*` NL→SQL routes were removed from the product on 2026-07-31 (see [edge-cases.md](edge-cases.md#nl-sql)), so client-side is the path. Always show the user the SQL you generated before running a write.
