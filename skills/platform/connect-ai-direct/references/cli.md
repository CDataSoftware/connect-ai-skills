...
| Command | Purpose |
|---|---|
| `oauth-start --driver D --name N [--props '<json>']` | BFF handshake step 1: returns the provider consent URL + `callbackId`, stashes the pending state. Full flow + per-provider decode rules: see the `connect-ai-manage` skill. |
...
| `roles` | List roles — integer id = system role, UUID = custom role (guided flows in the `connect-ai-manage` skill). |
| `user-invite --email E --role N [--custom-role-ids '<json>'] [--permissions '<json>']` | Send an invite (`POST /api/ui/user/inviteNewUserList`). Use the guided flow in the `connect-ai-manage` skill to collect the inputs. |
...
| `usage` | Current usage stats (rich widget template in the `connect-ai-manage` skill). |
...
| `assets-add --workspace-id ID --connection-id ID --schema S --tables T1,T2` | Publish tables from a connection (batch). Guided multi-step flow: see the `connect-ai-manage` skill. |

### Toolkits
| Command | Purpose |
|---|---|
| `toolkits` · `toolkit-create --name N` · `toolkit-tools --id ID` | List / create / list tools. Each data source = 2 backend records (universal + source) — see the `connect-ai-manage` skill. |
...
> The bundled **Python helpers** `scripts/cdata_workspaces.py` and `scripts/cdata_jobs.py` cover the same ground with richer output (grouped tool views, `claude mcp add --run`, table formatting) — see the `connect-ai-manage` skill (workspaces & jobs). They share the CLI's token cache, so one `login` serves all three. (Windows: if `python` isn't on PATH, use `py`.)

### Escape hatch
| Command | Purpose |
|---|---|
| `raw --method M --path /api/... [--body '<json>'] [--query '<json>']` | Call any endpoint not yet wrapped. `DELETE` requires `--confirm`. Use this for the long tail (full endpoint catalog in the `connect-ai-manage` skill). |
...
- **paste a Bearer token** and call the REST endpoints in [operations.md](operations.md) with the host's HTTP tool (full endpoint catalog in the `connect-ai-manage` skill), or
